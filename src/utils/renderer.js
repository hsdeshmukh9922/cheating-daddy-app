...
// ============ SMART CLICK-THROUGH ============
// While click-through mode is on (live/assistant view), clicks pass through the
// overlay to the app underneath (notepad, browser, etc.). Hovering a real control
// (button, input, link...) re-enables mouse events so the overlay stays usable.
let clickThroughMode = false;
let lastIgnoreSent = null;
const INTERACTIVE_TAGS = new Set(['BUTTON', 'INPUT', 'TEXTAREA', 'SELECT', 'A', 'LABEL', 'OPTION']);
const INTERACTIVE_CLASSES = ['drag-region', 'traffic-light', 'traffic-lights', 'nav-btn', 'input-bar', 'live-bar', 'response-nav', 'copy-btn'];

function isInteractiveTarget(e) {
    return e.composedPath().some(el => {
        if (!el || el.nodeType !== 1) return false;
        if (INTERACTIVE_TAGS.has(el.tagName)) return true;
        if (el.hasAttribute && el.hasAttribute('data-interactive')) return true;

        // Scrollbar hover area detection on the response container
        if (el.classList && el.classList.contains('response-container')) {
            const rect = el.getBoundingClientRect();
            const mouseX = e.clientX;
            // Scrollbar width is 6px, allow 16px hover zone for easy scroll dragging
            if (mouseX >= rect.right - 16) {
                return true;
            }
        }

        return INTERACTIVE_CLASSES.some(cls => el.classList && el.classList.contains(cls));
    });
}
...
window.addEventListener(
    ...
    e => {
        ...
        const shouldIgnore = !isInteractiveTarget(e);
        if (shouldIgnore !== lastIgnoreSent) {
            lastIgnoreSent = shouldIgnore;
    ...
    e => {
        ...
        if (app && app.shadowRoot) {
            ...
            if (assistantView && assistantView.shadowRoot) {
                const container = assistantView.shadowRoot.querySelector('.response-container');
                if (container) {
...
ipcRenderer.on('click-through-toggled', (event, enabled) => {
    clickThroughMode = enabled;
    // Main process just set the window-level state; reset hover tracking
    lastIgnoreSent = enabled ? true : null;
    console.log('Click-through mode:', enabled ? 'on (hover controls to interact)' : 'off');
});
